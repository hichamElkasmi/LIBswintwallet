...
import com.ssw.bnk.swintwallet.jdbc.entities.SSEnt_Trasansaction;
 ...
 * @author MOKHLISS
 */
public class SSRes_TransactionsList extends SSRes_Base{
    ...
    public SSRes_TransactionsList() {
        super();
        transactions = null;
    }
    
    public SSRes_TransactionsList(SSEnt_Auth sSEnt_Auth) {
        super(sSEnt_Auth);
        transactions = null;
    }
    
    public SSRes_TransactionsList(SSEnt_Header header, SSEnt_Trasansaction[] transactions, SS_Stt status) {
        super(header,status);
    ...
    public void parseTrxList(){
        ...
        try {
                ...
                if(objstructs == null || objstructs.length==0){
                    ...
                    transactions = new SSEnt_Trasansaction[objstructs.length];
                    int i =0;
                    for(Object obj : objstructs){
                           Struct s= (Struct)obj;
                            if(s != null ){
                                   ...
                                   try{
                                        Instant instant = ((Timestamp)str[2]).toInstant() ;
                                        DateFormat dateFormat = new SimpleDateFormat("dd/MM/YY hh:mm:ss");
                                        strDate = dateFormat.format(Date.from(instant));
                                    }catch (Exception ex) {
                                        strDate = null;
                                    }
                                   transactions[i] =  new SSEnt_Trasansaction((String)str[6],strDate,(String)str[3],
                                           ((BigDecimal)str[4]).toString(), (String)str[5], (String)str[1]);
                             }
                            i++;
                       }
                }
